...
Read the relevant files and run checks appropriate to the type. See `~/.claude/skills/verify/check-types.md` for the four composable check categories (data, completeness, consistency, assumption). A single verification usually draws from multiple categories — e.g., a data pipeline verification runs data + completeness + consistency.
...
If Step 3 reports FAILED items, do not jump to fixing. See `~/.claude/skills/verify/debug-protocol.md` for the full 4-phase protocol.
